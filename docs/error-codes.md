...
| `BAD_CREDENTIAL_RECORD` | `AccessErrorCode` | One record is unusable: an id outside `[A-Za-z0-9_-]{1,64}`, an unknown kind, a public key or secret hash of the wrong width, a `bearer` secret hash that is the digest of no bytes, a `scopes` list that grants `complete` without `read`, an unknown scope, a missing `createdAt`, or at request time a `pop` record that carries no key its kind can be verified against | Fix that record. A `bearer` hash comes from the generator, which writes the digest of 32 random bytes and never the digest of none. A `scopes` list that grants `complete` alone is fixed by adding `read`, or by dropping `complete` if the credential only sends. The request-time form names the id, because the file and the request are both fine and only the record is not | terminal |
...
| `AUTH_UNKNOWN` | `AccessErrorCode` | Nothing in the file answers to the credential the header names, or no bearer digest matches the secret presented. On the bearer path one refusal does cover a secret matching no stored digest and a secret belonging to a retired record, because the digest scan passes over a revoked record as though it had never existed. On the proof-of-possession path it does not: an id that resolves to a record is answered by whichever check follows, so `AUTH_UNKNOWN` there says the file carries no record under that id at all, which is the residual `docs/threat-model.md` row T15 records | Check the id against the credential file the operator holds. A 401 here says the deployment has never heard of this credential, which is not the same message as `AUTH_REVOKED` | terminal |
