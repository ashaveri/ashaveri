...
about it. There are 117 declarations across seven unions, resolving to 115 distinct strings;
...
| Union | Package | Owns |
|---|---|---|
| `ReceiptErrorCode` | `@ashaveri/receipt` | The COSE receipt wire format, the export container and the PoP Authorization header: decoding, signature, payload and manifest fields |
...
| `UNSUPPORTED_ALG` | `ReceiptErrorCode` | The protected header's `alg` is not an integer label, or is an integer that is not EdDSA. A header whose `alg` arrives as a floating-point number is answered under `BAD_PROTECTED_HEADER` instead, and before this read: that is the header failing to decode under the format's rule for it, not a parameter naming a suite | Refuse; the receipt claims a suite this spec does not define | terminal |
| `BAD_PROTECTED_HEADER` | `ReceiptErrorCode` | The header map is not a map, does not decode under the rules this format sets for it, carries a label it is not one of the three `receipt.cddl` declares, or `kid` is not 32 bytes or `typ` is not the receipt content type. Not decoding is the floating-point case, and it is this code rather than the code of the parameter the float would have reached: a label written as the half-float `1.0` takes the same map slot as the integer `1`, and core deterministic ordering always writes the integer first and the float last, so the value a reader is handed is the float's and which parameters the issuer signed is no longer a question the map can answer. An undeclared label is named in the message, and refusing one is not a strictness habit: the protected bytes are what the `Sig_structure` hashes, so a reader that took the three it knows and returned those would hand on a document other than the one the issuer signed | Refuse; this is a signed envelope for something else, or for this one with a parameter added to what the signature covers | terminal |
...
| `BAD_PAYLOAD` | `ReceiptErrorCode` | A payload field is missing or mis-typed: a `v` that is not an integer, a `v: 2` document with no `mk`, a member the format makes a map and is not one, an integer where the format promises a non-negative one. Also a document carrying a member its payload version does not define, or one of the maps nested inside it carrying a member the format does not define, which every map being closed makes a refusal rather than a member left out, and a measurement whose width disagrees with its kind, at issue time. A floating-point number anywhere in the payload or the maps inside it answers this code too, and from the decode rather than from a field read: the float `128.0` and the integer `128` are one JavaScript number by the time a check could tell them apart, so the distinction is settled while the bytes are being read, which is as wide as a payload whose every member the CDDL names and none of them as a float | Refuse; a signed garbage payload is still garbage | terminal |
...
| `MARK_MISMATCH` | `ReceiptErrorCode` | `sha256` of the marked region, taken by the extraction rule `mk.sch` names, does not equal the `d` this receipt signed. The region is read off the response bytes by `extractMarkedRegion` in `@ashaveri/receipt`, and the live site that reaches this code is `@ashaveri/sdk`'s `verifyCompletionReceipt`, which is handed those bytes beside their digest; `verifyReceipt` and `decodeReceipt` still do not reach it, because neither is handed the stream the region is carved out of. Both the absence of a region and more than one are this code rather than a choice between candidates | Refuse; the response carries a mark that is not the one the receipt attests. Deleting the mark is a different failure and announces itself as the response no longer hashing to `res` | terminal |
...
| `EXPORT_MALFORMED_CBOR` | `ReceiptErrorCode` | The bytes handed to the export reader do not decode as canonical CBOR, so no envelope and no manifest is reached. The receipt's own `MALFORMED_CBOR` is not used here because its fixed sentence names receipt bytes, and a log line carries only the string | Refuse; these are not export bytes. Check what re-encoded or truncated them | terminal |
| `EXPORT_BAD_HEADER` | `ReceiptErrorCode` | The export's signed header holds no map, does not decode under the rule `export.cddl` sets for it, carries a label outside the three it declares, gives a `kid` of another width, or names a content type that is not `ashaveri/export`, which is how a pack document and a receipt document are refused before one member of their payload is read. `alg` keeps its own answer | Refuse; this is a signed envelope for one of the other two containers, or for an export with a parameter added to what the signature covers | terminal |
| `EXPORT_UNSUPPORTED_VERSION` | `ReceiptErrorCode` | The export manifest's `v` is an integer no version of this container has used. A `v` that is not an integer at all is a malformed manifest, so it is `EXPORT_BAD_MANIFEST`, and the two answers differ for the same reason the receipt's do: one is about the reader's reach and the other about the bytes | Refuse; this is not an export this verifier was told to read. Nothing re-asks with the other reading | terminal |
| `EXPORT_BAD_MANIFEST` | `ReceiptErrorCode` | The export manifest, or a map inside it or inside the arm of a choice the document took, is absent, mis-typed, of the wrong width, negative where an instant or a count cannot be, empty where the arm it sits under declares a non-empty array, carrying a member this version does not define, holding a companion name that is a path rather than a name, or stating two members that contradict each other, which is the class `claim.made` after `at` belongs to. A floating-point number anywhere in the manifest or the signed header answers here too, and from the decode rather than from a field read, because by the time a check could tell `1` from `1.0` the signed bytes have become one map entry | Refuse; a signed handover whose manifest does not add up is still not evidence | terminal |
| `EXPORT_UNSUPPORTED_LABEL` | `ReceiptErrorCode` | A `k` or a `kind` names a shape or a statement this version defines no reading for: an arm of a collection or of an original, an assessment label beside `none`, or a claim kind outside `provenance` and `custody`. The detail says which position could not be read, and the refusal is not a guess about which label the writer meant, which is the confusion this code exists to prevent | Refuse; the document is well-formed and from a vocabulary this reader does not have. Read it with a verifier for its own version, or ask the writer which statement was meant | terminal |
| `EXPORT_KID_MISMATCH` | `ReceiptErrorCode` | The header's `kid` differs from the sha256 of the public key the reader was handed. The reader takes one key and resolves nothing, so this is an answer about the configuration it was given rather than about the bytes | Supply the key the header names, from the deployment's own published material | terminal |
| `EXPORT_DUPLICATE_ID` | `ReceiptErrorCode` | Two items of one collection answer to the same `id`. The links order a run and the name identifies an item, so a refusal that reports an item by name would otherwise mean whichever of the two a reader happened to meet first, and nothing about a chain says the names collided | Refuse; the handover cannot be reported on item by item. Ask for the collection again | terminal |
| `EXPORT_DIGEST_MISMATCH` | `ReceiptErrorCode` | sha256 of an item's original bytes, inline or handed over as a companion, is not the `d` the item carries. The signature is usually fine here, which is the point: this is material edited after it was assembled, between the deployment and the reading | Refuse; the original is not the one the export vouches for. The refusal names the item | terminal |
| `EXPORT_ORIGINAL_UNAVAILABLE` | `ReceiptErrorCode` | An item references a companion file by name and the reader was not handed bytes under that name, so the digest beside it cannot be checked at all. Not a fault of the document: an unchecked original is refused by name rather than passed over, because a reader that skipped it would report on material it never looked at | Supply the companion file the item names, then read again. Nothing about the export changes | terminal |
| `EXPORT_CHAIN_BROKEN` | `ReceiptErrorCode` | The walk over an anchored collection does not close: an item names a predecessor nobody in the collection is, two items claim the same predecessor so the run forks, or the recomputed digests stop short of the `head` the manifest names. The detail says which of the three it found | Refuse; the run is not what it claims to be. A record lifted out of the middle and a hand-edited one look the same from here | terminal |
| `EXPORT_ITEM_UNREACHED` | `ReceiptErrorCode` | An item of an anchored collection lies outside the run from the anchor to the head, so the walk reached the endpoint without visiting it. This is the half of the rule the head cannot see, and it is why a conforming reader counts what it walked against the array it was handed | Refuse; the collection carries material that is not part of the run it claims. The refusal names the items | terminal |
| `EXPORT_ENDPOINT_MISMATCH` | `ReceiptErrorCode` | The reader was given an anchor or a head it already had cause to believe and the document names a different one, or the reader was given one and the collection states no endpoints at all, which a pin is never allowed to pass silently. The document may be whole: this is a finding about which of the two the reader is holding | Compare the two endpoints. This is how a deletion becomes visible, and only against a copy held from before | terminal |
