...
| `BAD_PROTECTED_HEADER` | `ReceiptErrorCode` | The header map is not a map, a label it carries is not one of the three `receipt.cddl` declares, `kid` is not 32 bytes, or `typ` is not the receipt content type. An undeclared label is named in the message, and refusing one is not a strictness habit: the protected bytes are what the `Sig_structure` hashes, so a reader that took the three it knows and returned those would hand on a document other than the one the issuer signed | Refuse; this is a signed envelope for something else, or for this one with a parameter added to what the signature covers | terminal |
