...
- **A7 The marked response.** A response whose own bytes carry a machine-readable marking: one extra
  ...
  without the edit showing. Of the three pieces, two are unbuilt and one is not. No member and no
  frame is written into any response here: the gateway issues `v: 1` (`gateway/src/server.ts`), and
  section 6 says what follows for a deployment. The field is built: `@ashaveri/receipt` reads a
  `v: 2` payload that carries `mk` and refuses a `v: 1` payload that carries one, because a payload
  map is closed at either version ([receipt-spec.md](receipt-spec.md) section 6), so the marking
  attestation cannot arrive on a document that has agreed to say nothing about it. Reading the field
  is the whole of what that gives a verifier today: no extractor, no region check (T18, T19).
- **A8 The marking-scheme registry.** The table that binds every scheme label to exactly one byte
  ...
  look for, is unbuilt too, and the parser's accepted set is not a substitute for it: two labels,
  `"none"` and `"provenance-v1"`, named in code rather than read off a published table (T19). It is an
  asset because its failure mode is silent: a label standing for two shapes turns each detector's
  answer into a guess about which shape it happened to read.
...
- **A mark survives a client in the shape a client reads, and not in the shape of a bare frame.** What
  ...
  member intact, on the object a caller holds and on the bytes it re-serialises, and a streamed
  response carrying the same member on a final chunk whose `choices` is empty reaches both the raw
  iterator and the official accumulator the same way. A `data:` frame that is neither a chunk nor the
