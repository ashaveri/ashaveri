/**
 * The two vendor roots a remote verifier needs to decide that platform evidence
 * came from real hardware, embedded as defaults so verification works offline.
 *
 * These are public certification authorities, not secrets, and pinning them is
 * what makes an evidence chain meaningful: without a local anchor a certificate
 * chain only proves that some key signed something. A deployment that keeps its
 * own copy passes it through the verification options instead, and this table
 * then plays no part.
 *
 * Only the anchor is pinned here, never any collateral: TCB Info, QE Identity,
 * SKR, and the AMD CRL still live at Intel PCS and AMD KDS and are not fetched.
 */

/**
 * Intel SGX Root CA, the anchor every PCK chain must reach.
 *
 * Downloaded from
 * https://certificates.trustedservices.intel.com/Intel_SGX_Provisioning_Certification_RootCA.pem
 * serial `22650cd65a9d3489f383b49552bf501b392706ac`, SHA-256 fingerprint
 * `44a0196b2b99f889b8e149e95b807a350e7424964399e885a7cbb8ccfab674d3`,
 * ECDSA P-256, valid 2018-05-21 to 2049-12-31.
 */
export const INTEL_SGX_ROOT_CA_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

/**
 * AMD SEV Root Key for the Milan product line, the anchor an ARK to ASK to VCEK
 * chain must reach.
 *
 * CN=ARK-Milan, serial `010000` (65536), SHA-256 fingerprint
 * `69d063b45344d26a2e94e1f4210de49ef555308287d4c174445c95639a540bcd`,
 * RSA-4096, valid 2020-10-22 to 2045-10-22. Rome and Genoa have their own ARK,
 * so a fleet that spans product lines needs its own anchor list.
 */
export const AMD_ARK_MILAN_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

function pem(text: string): Uint8Array {
  return new TextEncoder().encode(text);
}

/** The bundled Intel SGX root, in the shape `VerifyOptions.trustedIntelRoots` takes. */
export const DEFAULT_INTEL_SGX_ROOTS: readonly Uint8Array[] = [pem(INTEL_SGX_ROOT_CA_PEM)];

/** The bundled AMD Milan ARK, in the shape `VerifyOptions.trustedArks` takes. */
export const DEFAULT_AMD_ARKS: readonly Uint8Array[] = [pem(AMD_ARK_MILAN_PEM)];
